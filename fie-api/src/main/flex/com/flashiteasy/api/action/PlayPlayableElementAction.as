...
package com.flashiteasy.api.action
{
	 ...
	 */
	public class PlayPlayableElementAction extends Action implements IElementAction, ITimedElementAction
	{
		 ...
		 * Constructor
		 */
		public function PlayPlayableElementAction()
		{
			super();
		}
		...
		override public function apply( event : Event ):void
		{
			for each (var uuid : String in _elementList)
			{
				var element : IDescriptor = ElementList.getInstance().getElement(uuid, elementsPage);
				 ...
				 {
				 	IPlayableElementDescriptor(element).gotoTimeAndPlay( time );
				 }
			}
		}
		...
		{
				return PlayPlayableElementAction;
		}
