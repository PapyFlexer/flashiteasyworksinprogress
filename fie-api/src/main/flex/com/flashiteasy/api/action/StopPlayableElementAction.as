...
package com.flashiteasy.api.action
{
	...
	/**
	 *
	 * The <code><strong>StopPlayableElementAction</strong></code> class defines an Action that triggers
	 * one or more "playable" Elements (swfs, videos, ...) on stage
	 */
	public class StopPlayableElementAction extends Action implements IElementAction, ITimedElementAction
	{

		/**
		 *
		 * @default : the Array of elements to stop
		 */
		 ...
		 * Constructor
		 */
		public function StopPlayableElementAction()
		{
			super();
		}
		...
		override public function apply(event:Event):void
		{
			for each (var uuid:String in _elementList)
			{
				...
				{
					IPlayableElementDescriptor(element).gotoTimeAndStop(_time);
				}
			}
		}
		...
		/**
		 *
		 * @return the time where to go and play in the targetted control
		 * Getter
		 */
		...
		/**
		 *
		 * @param value the time where to go and play in the targetted control
		 * Setter
		 */
		...
		{
			return StopPlayableElementAction;
		}
