...
- example-lib : example de librairie externe chargée au runtime(clock-lib, clouds-lib, etc.). C'est le type de modules fourni aux développeurs pour cherger leurs propres contenus au runtime et  rendre éditable les paramètres souhaités en fonction de leur typage (int, number, string, list, xmllist, éléments de formulaires, etc.)
